...
/*
    ...
    function _provable_request(uint256 requestId, uint256 network, address tokenForeign, address user) internal {
        ...
        string memory a = "json(https://api-ropsten.etherscan.io/api?module=account&action=tokenbalance&contractaddress=0x";
        string memory b = "&address=0x";
        string memory c = "&tag=latest&apikey=WQRMM9PRE1CZ1VFSMTKUG5EP6JD8378E2A).result";
        string memory s = strConcat(a,_address2hex(token),b,_address2hex(user),c);
        LogMsg(s);
        provable_query("URL", s);
    }
