...
contract Validator is Ownable {
    ...
    function getRate() internal view returns(uint256 rate) {
        uint256 bnb = IPrice(currencyPrice).getCurrencyPrice(address(0));
        uint256 eth = IPrice(currencyPrice).getCurrencyPrice(address(1));
        require(bnb != 0 && eth != 0, "Zero price");
        rate = eth * NOMINATOR / bnb;
    }
